...
export function Stepper({
  ...
  const getCircleClass = (state: string) => {
    if (state === "completed") return cn(completedColor, completedTextColor)
    if (state === "active") return cn(activeColor, activeTextColor)
    return cn(inactiveColor, inactiveTextColor)
  }
  ...
  const renderCircle = (item: StepItem, index: number) => {
    ...
    return (
      <div
        className={cn(
          "shrink-0 inline-flex items-center justify-center rounded-full font-semibold transition-all duration-300",
          s.circle,
            ...
            <div key={index} className="flex gap-4">
              <div className="flex flex-col items-center">
                ...
                {!isLast && (
                  <div
                    className={cn(
                      "w-0.5 flex-1 my-1 transition-all duration-300",
                      state === "completed" ? completedConnectorColor : connectorColor,
                    )}
                  />
                )}
              </div>
              <div className={cn("pb-6")}>
                <p className={cn("font-medium", s.label, state === "inactive" ? inactiveTextColor : labelColor)}>
                  {item.label}
                </p>
                {item.description && (
                  <p className={cn("mt-0.5", s.desc, descriptionColor)}>
                    {item.description}
                  </p>
                )}
              </div>
          ...
          <React.Fragment key={index}>
            <div className="flex flex-col items-center gap-2 shrink-0">
              ...
              <div className="text-center">
                ...
                {item.description && (
                  <p className={cn("mt-0.5", s.desc, descriptionColor, "max-w-25")}>
                    {item.description}
                  </p>
                )}
              </div>
