...
const toastStyles: Record<ToastType, { bg: string; iconColor: string; icon: React.ReactNode }> = {
  success: {
...
export function ToastContainer({ position = "top-right" }: ToastContainerProps) {
  ...
  return (
    <div className={cn("fixed z-9999 flex flex-col gap-2 pointer-events-none", positionStyles[position])}>
      {items.map((item) => {
