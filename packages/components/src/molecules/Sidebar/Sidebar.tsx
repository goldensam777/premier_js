...
export function Sidebar({
  ...
  const renderItems = (list: SidebarItem[], depth = 0) =>
    list.map((item, i) => {
      if (item.divider) {
        return (
          <div key={i} className={cn("my-2 border-t", borderColor)} />
        )
      }
      ...
      const baseClass = cn(
        "flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 text-left",
        depth > 0 ? "ml-4 w-[calc(100%-1rem)]" : "",
        item.disabled ? "opacity-40 cursor-not-allowed" : "cursor-pointer",
        isActive
          ? cn(activeColor, activeTextColor)
          : cn(inactiveTextColor, "hover:bg-gray-50", `hover:${activeTextColor}`),
      )
      ...
      return (
        <div key={item.value}>
          ...
          >
            {item.icon && (
              <span className={cn("shrink-0 w-5 h-5", isActive ? activeTextColor : iconColor)}>
                {item.icon}
              </span>
            )}
            ...
            {!collapsed && hasChildren && (
              <svg
                className={cn("w-4 h-4 shrink-0 transition-transform duration-200", isExpanded ? "rotate-180" : "", iconColor)}
  ...
  return (
    ...
    >
      {logo && (
        <div className={cn("flex items-center px-4 py-5 border-b", borderColor, collapsed ? "justify-center" : "justify-between")}>
          {logo}
          {collapsible && (
            <button
