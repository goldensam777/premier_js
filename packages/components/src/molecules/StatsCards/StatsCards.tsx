          ...
          {items.map((item, i) => (
            <React.Fragment key={i}>
              <div className="flex flex-col items-center text-center px-10 py-6">
                ...
                <span className="mt-1 text-sm font-semibold uppercase tracking-wide" style={{ color: labelColor }}>
                  {item.label}
                </span>
                {item.description && (
              ...
              {showDividers && i < items.length - 1 && (
                <div className="hidden md:block h-16 w-px" style={{ backgroundColor: "var(--color-border)" }} />
              )}
            </React.Fragment>
          ))}
        </div>
